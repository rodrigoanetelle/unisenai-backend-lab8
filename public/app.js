...
//============================================================================
// BLOCO 1: MOTOR DE BIG DATA (MANTIDO TEMPORARIAMENTE PARA A TELA NÃO QUEBRAR)
//============================================================================
const gerarBigData = () => {
    const banco = {
    ...
    for (let i = 1; i <= 10000; i++) {
        ...
        banco.disciplinas.filter(d => d.semestre === semestreId).forEach(disc => {
            banco.notas.push({ id: notaIdCount++, alunoId: aluno.id, disciplinaId: disc.id, valor: parseFloat((Math.random() * 6 + 4).toFixed(1)), faltas: Math.floor(Math.random() * 12) });
        });
    }
    return banco;
};
...
const Grafico = ({ tipo, dados, opcoes }) => {
    ...
    useEffect(() => {
        ...
        chartInstance.current = new Chart(canvasRef.current.getContext('2d'), { type: tipo, data: dados, options: opcoes });
...
//============================================================================
// BLOCO 2: TELA DE AUTENTICAÇÃO CONECTADA AO RENDER
//============================================================================
const TelaLogin = () => { 
    ...
    const processarLogin = async (e) => {
        ...
        try {
            // Integração Real com a sua API na Nuvem
            const resposta = await fetch('https://unisenai-backend-lab8-2.onrender.com/login', {
                method: 'POST',
                headers: {
            ...
            if (resposta.ok && dados.token) {
                // Guarda o Token Real devolvido pelo seu Node.js
                localStorage.setItem('jwt_token', dados.token); 
                navigate('/dashboard'); 
            } else {
                setErro(dados.erro || "Credenciais inválidas ou erro no servidor.");
            }
        } catch (error) {
            setErro("Falha ao comunicar com o servidor Render. Verifique a sua conexão.");
        } finally {
            setCarregando(false);
        }
    };
        ...
        <div className="vh-100 d-flex align-items-center justify-content-center" style={{ background: "url('https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=1920&q=80') center/cover", backgroundColor: "rgba(15, 23, 42, 0.85)", backgroundBlendMode: "overlay" }}>
            <div className="glass-effect p-5 fade-in w-100 mx-3" style={{ maxWidth: '450px' }}>
                <div className="text-center mb-4">
                    ...
                    <p className="text-muted small fw-semibold mt-1">UniSENAI 106 - Módulo Administrativo</p>
                </div>
                ...
                <form onSubmit={processarLogin}>
                    <div className="mb-3">
                        ...
                        <div className="input-group shadow-sm">
                            ...
                            <input type="email" className="form-control py-2 border-start-0 ps-0" placeholder="admin@senai.br" value={email} onChange={e => setEmail(e.target.value)} disabled={carregando} required />
                        </div>
                    </div>
                    <div className="mb-4">
                        ...
                        <div className="input-group shadow-sm">
                            ...
                            <input type="password" className="form-control py-2 border-start-0 ps-0" placeholder="••••••••" value={senha} onChange={e => setSenha(e.target.value)} disabled={carregando} required />
                        </div>
                    </div>
                    ...
                    <button type="submit" className="btn btn-primary w-100 py-3 fw-bold shadow-sm d-flex justify-content-center align-items-center gap-2" disabled={carregando}>
                        {carregando ? (
                            <><div className="spinner-border spinner-border-sm text-light" role="status"></div> Autenticando...</>
                        ) : (
...
const Dashboard = ({ db, onLogout }) => {
    ...
    const [ordenacao, setOrdenacao] = useState({ coluna: 'nome', direcao: 'asc', disciplinaId: null });
    
    const dadosFiltrados = useMemo(() => {
        let alunos = db.alunos.filter(a => a.cursoId === cursoSelecionado && a.semestreId === semestreSelecionado);
        if (turnoSelecionado !== 'Todos') alunos = alunos.filter(a => a.turno === turnoSelecionado);
        alunos.sort((a, b) => {
            if (ordenacao.coluna === 'nome') return ordenacao.direcao === 'asc' ? a.nome.localeCompare(b.nome) : b.nome.localeCompare(a.nome);
            if (ordenacao.coluna === 'nota' && ordenacao.disciplinaId) {
                const notaA = db.notas.find(n => n.alunoId === a.id && n.disciplinaId === ordenacao.disciplinaId)?.valor || 0;
                const notaB = db.notas.find(n => n.alunoId === b.id && n.disciplinaId === ordenacao.disciplinaId)?.valor || 0;
    ...
    const alternarOrdenacao = (coluna, disciplinaId = null) => {
        setOrdenacao({ coluna, direcao: ordenacao.coluna === coluna && ordenacao.direcao === 'asc' ? 'desc' : 'asc', disciplinaId });
    };
    
    const renderIcone = (coluna, disciplinaId = null) => {
        if (ordenacao.coluna !== coluna || ordenacao.disciplinaId !== disciplinaId) return <i className="fa-solid fa-sort text-muted ms-2 opacity-25"></i>;
            ...
            <nav className="navbar navbar-dark bg-dark shadow-sm px-4 py-3">
                <div className="d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <small className="text-secondary fw-semibold" style={{fontSize: '0.75rem'}}>Engine de Big Data • Híbrido</small>
                    </div>
                </div>
                <button onClick={onLogout} className="btn btn-outline-light btn-sm fw-bold"><i className="fa-solid fa-power-off me-2"></i>Sair</button>
            </nav>
            <main className="container-fluid py-4 flex-grow-1" style={{ maxWidth: '1400px' }}>
                <div className="card shadow-sm border-0 mb-4 p-3 rounded-4 bg-white">
                    <div className="row g-3 align-items-center">
                        <div className="col-md-4">
                            <label className="form-label small fw-bold text-muted mb-1">Selecione o Curso</label>
                            <select className="form-select bg-light" value={cursoSelecionado} onChange={(e) => setCursoSelecionado(Number(e.target.value))}>
                        ...
                        <div className="col-md-2">
                            ...
                            <select className="form-select bg-light" value={semestreSelecionado} onChange={(e) => setSemestreSelecionado(Number(e.target.value))}>
                                {db.periodos.map(p => <option key={p.numero} value={p.numero}>{p.nome}</option>)}
                            </select>
                        </div>
                        <div className="col-md-3">
                            ...
                            <select className="form-select bg-light border-primary" value={turnoSelecionado} onChange={(e) => setTurnoSelecionado(e.target.value)}>
                ...
                <ul className="nav nav-pills mb-4">
                    <li className="nav-item">
                        <button onClick={() => setAbaAtiva('tabela')} className={`nav-link px-4 ${abaAtiva === 'tabela' ? 'active shadow-sm' : 'bg-white shadow-sm'}`}>
                            <i className="fa-solid fa-table-list me-2"></i> Tabela Analítica (Sortable)
                        </button>
                    </li>
                    <li className="nav-item">
                        <button onClick={() => setAbaAtiva('analytics')} className={`nav-link px-4 ${abaAtiva === 'analytics' ? 'active shadow-sm' : 'bg-white shadow-sm'}`}>
                ...
                {abaAtiva === 'tabela' && (
                    <div className="card shadow-sm border-0 rounded-4 overflow-hidden fade-in">
                        <div className="bg-primary-subtle p-3 border-bottom border-primary d-flex align-items-center justify-content-between">
                            <div>
                                <h6 className="fw-bold text-primary-emphasis mb-0"><i className="fa-solid fa-sort me-2"></i>Ordene os dados clicando no cabeçalho</h6>
                                <small className="text-primary-emphasis">Amostragem: Visualizando os 100 primeiros registros.</small>
                            </div>
                        </div>
                        <div className="table-responsive" style={{ maxHeight: '600px' }}>
                            <table className="table table-hover align-middle mb-0 text-sm">
                                <thead className="table-light sticky-top shadow-sm">
                                    <tr>
                                        <th className="px-4 cursor-pointer bg-white" onClick={() => alternarOrdenacao('nome')} title="Clique para ordenar por nome">
                                            Aluno / RA {renderIcone('nome')}
                                        </th>
                                        <th className="px-4 bg-white">Turno</th>
                                        {dadosFiltrados.disciplinas.slice(0,4).map(d => (
                                            <th key={d.id} className="text-center cursor-pointer bg-white" onClick={() => alternarOrdenacao('nota', d.id)}>
                                                <div className="text-truncate" style={{maxWidth: '120px'}} title={d.nome}>{d.nome}</div>
                                                <div className="text-muted small fw-normal">(Nota) {renderIcone('nota', d.id)}</div>
                                            </th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                                    {dadosFiltrados.alunos.slice(0, 100).map(aluno => (
                                        <tr key={aluno.id}>
                                            ...
                                            <td className="px-4"><span className="badge bg-secondary-subtle text-secondary">{aluno.turno}</span></td>
                                            {dadosFiltrados.disciplinas.slice(0,4).map(disc => {
                                                const nota = db.notas.find(n => n.alunoId === aluno.id && n.disciplinaId === disc.id);
                                                const isSorted = ordenacao.coluna === 'nota' && ordenacao.disciplinaId === disc.id;
                                                return (
                                                    <td key={disc.id} className={`text-center fw-bold ${isSorted ? 'bg-light' : ''}`}>
                                                        <span className={nota && nota.valor >= 6 ? 'text-success' : 'text-danger'}>{nota ? nota.valor.toFixed(1) : '-'}</span>
                                                    </td>
                                                );
                                            })}
                                        </tr>
                                    ))}
                                    {dadosFiltrados.alunos.length === 0 && <tr><td colSpan="10" className="text-center py-5 text-muted">Nenhum aluno encontrado para este filtro.</td></tr>}
                                </tbody>
                            </table>
                        </div>
                    </div>
                ...
                {abaAtiva === 'analytics' && (
                    <div className="card shadow-sm border-0 rounded-4 p-4 fade-in">
                        ...
                        <small className="text-muted d-block mb-4">Cálculo matemático executado em tempo real baseado no filtro atual.</small>
                        <div style={{ height: '400px' }}>
                            <Grafico tipo="bar" dados={{ labels: labelsDisciplinas, datasets: [{ label: 'Média de Notas', data: mediasNotas, backgroundColor: '#0d6efd', borderRadius: 4 }] }} opcoes={{ maintainAspectRatio: false, scales: { y: { max: 10 } } }} />
                        </div>
                    </div>
                )}
            </main>
